...
var _ActionConfirmationForm = require('./ActionConfirmationForm');
...
var QForm = function (_React$Component) {
    ...
    function QForm(props) {
        ...
        if (_this.props.entitiesName) {
            ...
            _this.validationCtrl = new _atomityCore.ValidationController({
        ...
        _this.cancel = function () {
            self.closeDialog();
            self.props.onDialogClose({ dialogResult: _atomityCore.DialogResult.cancel });
        };
        ...
        _this.doAction = function () {
            if (self.props.entitiesName && self.props.okAction) {
                ...
                self.dataProvider.executeAction(self.props.entitiesName, self.props.okAction, entityObject).then(function (result) {
                    self.closeDialog();
                    self.props.onDialogClose({ dialogResult: _atomityCore.DialogResult.ok, entityObject: self.props.entityObject });
                ...
                self.closeDialog();
                self.props.onDialogClose({ dialogResult: _atomityCore.DialogResult.ok });
            }
        };
        ...
        _this.ok = function () {
            if (self.props.disableConfirmation === true) {
                ...
                _atomityCore.DialogService.showDialog(_ActionConfirmationForm2.default).then(function (result) {
                    if (result.dialogResult === _atomityCore.DialogResult.ok) {
                        self.doAction();
                    }
                });
            }
...
exports.DialogResult = _atomityCore.DialogResult;
